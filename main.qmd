...
mapa %>%
  left_join(selected_districts, by = c("ORG8CODE"="Code")) %>%
  ggplot(aes(fill = Week_lifted)) +
  geom_sf() +
...
```{r}
df <- inner_join(df,selected_districts %>% select(-Name))
...
enrollmentbyracegender <- enrollmentbyracegender %>% inner_join(selected_districts %>% select(Code,Week_lifted), by = c("District Code" = "Code"))
```
...
selectedpopulations <- selectedpopulations %>% inner_join(selected_districts %>% select(Code,Week_lifted), by = c("District Code" = "Code"))
...
staffracegender <- staffracegender %>% inner_join(selected_districts %>% select(Code,Week_lifted), by = c("District/School Code" = "Code"))
